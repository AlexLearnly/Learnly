    ...
    <div className="min-h-screen bg-[#F2F7F4] flex flex-col font-sans">
      ...
      <div className="flex-1 overflow-y-auto px-4 md:px-6 py-6 flex flex-col gap-4 max-w-3xl w-full mx-auto">
        {messages.map((m, i) => (
          <div key={i} className={`flex gap-3 items-end ${m.role === 'user' ? 'flex-row-reverse' : ''}`}>
            ...
            <div
              className={`max-w-[80%] px-4 py-3 rounded-2xl text-sm leading-relaxed ${m.role === 'user' ? 'bg-forest text-white rounded-br-sm' : 'bg-white border border-forest/10 text-gray-700 rounded-bl-sm'}`}
      ...
      <div className="bg-white border-t border-gray-100 px-4 md:px-6 py-4 flex-shrink-0">
        <div className="max-w-3xl mx-auto flex gap-3 items-end">
          ...
          />
          <button onClick={() => send()} disabled={!input.trim() || loading} className="bg-forest text-white text-sm font-medium px-5 py-3 rounded-xl hover:bg-deep transition-colors disabled:opacity-40 flex-shrink-0">Send</button>
        </div>
      </div>
    </div>
  )
